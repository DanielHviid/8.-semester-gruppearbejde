...
#-------------------------------------------------------------
# apply knn after clustering
#-------------------------------------------------------------
clusterknn <- function(training, test, numberOfClusters, numberOfClasses = 10, numberOfFolds = 10, testPart = 1, K = 1)
{
  ...
  for( i in 0:(numberOfClasses-1)) {
    clusterData <- kmeans(trainingData[ trainingClasses == i, ],numberOfClusters)
    clusterCiphers[[i + 1]] <- clusterData$centers
    clusterLabels[[i + 1]] <- c(1:numberOfClusters)*0 + i
  }
  
  trainingClusterLabels <- factor(unlist(clusterLabels))
  trainingClusterData <- clusterCiphers[[1]]
  for( i in 2:numberOfClasses) {
    trainingClusterData <- rbind(trainingClusterData,clusterCiphers[[i]])
  }
  
  predictions <- knn(train = trainingClusterData, test = testData, cl = trainingClusterLabels, k=K)
...
#-------------------------------------------------------------
# apply clustering to isolated ciphers
#-------------------------------------------------------------
cipherClusters <- function(input, numberOfClusters, numberOfClasses = 10)
{
  classes = input[,1]
  data = input[,-1]
  ...
  for( i in 0:(numberOfClasses-1)) {
    clusterData <- kmeans(data[ classes == i, ],numberOfClusters)
    clusterCiphers <- rbind(clusterCiphers, clusterData$centers)
  }
  
  return(unlist(clusterCiphers))
}
